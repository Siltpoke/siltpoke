...
/**
 * One-time canonical `?repo=` seed redirect on GET / (Task 12).
 *
 * Mirrors tests/web/routes/memory-seed-redirect.test.ts: a fresh landing (no
 * `?repo=`) that resolves a project (sticky pin, in this fixture) is promoted
 * to the explicit URL via a single 302 — so the pick is shareable/bookmarkable
 * and survives a reload without re-guessing. The loop guard: a request that
 * already carries `?repo=` must never redirect again, even when it resolves
...
async function makeApp(): Promise<{ app: Hono; projHash: string }> {
  const home = mkdtempSync(join(tmpdir(), "siltpoke-home-seed-redirect-home-"));
  dirs.push(home);
  const projectRoot = mkdtempSync(join(tmpdir(), "siltpoke-home-seed-redirect-proj-"));
  ...
  const app = new Hono();
  mountHomeRoutes(app, { homeBase: home, secret: "" });
  return { app, projHash };
}

describe("GET / — one-time canonical ?repo= seed redirect", () => {
  test("bare landing with a pinned project 302s to ?repo=<hash>", async () => {
    ...
    expect(res.headers.get("location")).toBe(`/?repo=${projHash}`);
  });

  test("a request that already carries ?repo= is NOT redirected (loop guard)", async () => {
    const { app, projHash } = await makeApp();
    const res = await app.request(`/?repo=${projHash}`, { redirect: "manual" });
