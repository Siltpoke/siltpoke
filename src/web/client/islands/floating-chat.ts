...
export function makeFloatingChatData(
  ...
  return {
    ...
    async send(anchorDecision?: "freeze" | "continue") {
      ...
      // critique_anchor and anchor are siblings on the wire — mutually
      // exclusive BY CONSTRUCTION (an `if`/`else if` chain, not independent
      // `if`s): a conversation is created pinned to a critique OR a graph
      // node, never both, so at most one of these branches can ever fire for
      // a given conv — see the Conversation.critiqueAnchor doc comment.
