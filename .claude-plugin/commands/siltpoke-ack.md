---
description: Acknowledge a Siltpoke review. Neutral status flip — no XP, no Reflexion.
argument-hint: <critique-id>
---

The user wants to acknowledge a review without saying it was right
(forward) or wrong (dismiss). Args are in `$ARGUMENTS`.

If `$ARGUMENTS` is empty, tell the user to provide a review id and stop.
...
- `status_set: "acked"`: confirm the review was marked seen, no XP
